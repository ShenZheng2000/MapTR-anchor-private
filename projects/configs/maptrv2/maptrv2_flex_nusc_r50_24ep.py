...
# Flex: no bev_seg (tokens aren't a spatial grid) and no pv_seg (no depth)
aux_seg_cfg = dict(
    use_aux_seg=False,
    bev_seg=False,
    pv_seg=False,
...
# K scene tokens produced by FlexSceneEncoder
_num_scene_tokens_ = 900
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        transformer=dict(
            ...
            encoder=dict(
                type='FlexSceneEncoder',
                num_scene_tokens=_num_scene_tokens_,
                embed_dims=_dim_,
                num_layers=4,
                num_heads=8,
                num_cams=6,
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        ...
                        dict(
                            ...
                            num_heads=8,
                            dropout=0.1),
                        # MultiheadAttention instead of CustomMSDeformableAttention:
                        # Flex bev_embed is (K, B, C) -- no spatial grid, so
                        # deformable attn's reference-point sampling doesn't apply.
                        dict(
                            type='MultiheadAttention',
                            embed_dims=_dim_,
                            num_heads=8,
        ...
        loss_pv_seg=dict(type='SimpleLoss',
                         pos_weight=1.0,
                         loss_weight=0.0),),  # pv_seg disabled
...
# No depth loading -- FlexSceneEncoder needs no depth supervision
train_pipeline = [
...
data = dict(
    samples_per_gpu=1,  # NOTE: 1 for debug (FlexSceneEncoder full-attn is memory-heavy)
    workers_per_gpu=4,
    train=dict(
        type=dataset_type,
        data_root=data_root,
...
optimizer = dict(
    type='AdamW',
    lr=1e-4,  # NOTE: reduced from 6e-4 for Flex stability debug
    paramwise_cfg=dict(
        custom_keys={
